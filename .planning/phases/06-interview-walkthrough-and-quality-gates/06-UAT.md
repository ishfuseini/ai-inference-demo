...
evidence: |
  ...
  Note: Live inference call not executed (would require a real OpenRouter API key and incur cost); UI launch + config + tracing-disabled state verified.
...
Live inference call intentionally not executed (requires real API key + cost); all UI state, config, and framing confirmed via browser.
